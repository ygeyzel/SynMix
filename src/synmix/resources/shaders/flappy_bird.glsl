#version 330
// from https://www.shadertoy.com/view/ls2GRt
...
void pipe(inout vec3 col, vec2 p, float h) {
  ...
  if (ap.y > h) {
    ...
    if (dy < 60.0)
      ap.x *= 0.93;
    // outline
    col = mix(col, vec3(0.322, 0.224, 0.290), step(ap.x, 65.0)); 
      ...
      if (abs(dy - 57.5) > 7.5)
        gradient = max(
          0.0, 0.5 * cos(
            floor((p.x + 25.0) / 5.0)
            * 5.0
            * (0.026 - 0.006 * step(dy, 10.0))
          )
        );
      // pipe
      col = mix(
        col, 
        vec3(0.322, 0.506, 0.129) + gradient,
        step(ap.x, 60.0)
      ); 
    }
  }
...
vec3 color(int id) {
  ...
  if (id == 0)
    return vec3(0.0);
  if (id == 1)
    return vec3(0.320, 0.223, 0.289);
  if (id == 2)
    return vec3(0.996, 0.449, 0.063);
  if (id == 3)
    return vec3(0.965, 0.996, 0.965);
  if (id == 4)
    return vec3(0.996, 0.223, 0.000);
  if (id == 5)
    return vec3(0.836, 0.902, 0.805);
...
int sprite(vec2 p) {
  ...
  if (clamp(p.x, 0.0, 16.0) == p.x && clamp(p.y, 0.0, 11.0) == p.y) {
    ...
    if (int(mod(n / (pow(2.0, k)), 2.0)) == 1)
      d += 1;
    if (int(mod(n / (pow(2.0, k + 1.0)), 2.0)) == 1)
      d += 2;
    if (int(mod(n / (pow(2.0, k + 2.0)), 2.0)) == 1)
      d += 4;
  }
  return d;
}
...
void ground(inout vec3 col, vec2 p) {
  ...
  if (p.y < -285.0)
    col = color(1);
  if (p.y < -290.0)
    col = vec3(0.902, 1.000, 0.549);
  if (p.y < -295.0)
    col = mix(
      vec3(0.612, 0.906, 0.353), 
      vec3(0.451, 0.745, 0.192),
      step(mod(p.x - floor(p.y / 5.0) * 5.0, 60.0), 30.0)
    );
  if (p.y < -325.0)
    col = vec3(0.322, 0.506, 0.129);
  if (p.y < -330.0)
    col = vec3(0.839, 0.667, 0.290);
  if (p.y < -335.0)
    col = vec3(0.871, 0.843, 0.580);
}
